...
const seats = computed(() => {
  ...
  return seatData.map((seat) => ({
    ...
    occupied: props.occupied.includes(seat.id) || props.occupied.includes(seat.name),
  }));
});
...
function getSeatClasses(seat) {
  if (props.occupied.includes(seat.id.toString()) || props.occupied.includes(seat.name.toString())) {
...
function proceedToCheckout() {
  ...
  const seatNames = selectedSeats.value.map((seat) => seat.name);
